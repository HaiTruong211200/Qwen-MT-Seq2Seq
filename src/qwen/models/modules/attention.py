...
from .rotary_embedding import QwenRotaryEmbedding, QwenLinearScalingRotaryEmbedding, QwenDynamicNTKScalingRotaryEmbedding
from .rotary_embedding import apply_rotary_pos_emb, apply_rotary_pos_emb_single
...
class QwenFlashAttention2(QwenAttention):
    """
    Qwen flash attention module. This module inherits from `QwenAttention` as the weights of the module stays
    ...
    def forward(
        ...
        **kwargs,
        ...
        # in fp32. (QwenRMSNorm handles it correctly)
    ...
    def _flash_attention_forward(
        ...
        if not self._flash_attn_uses_top_left_mask:
            ...
            # TODO: Remove the `query_length != 1` check once Flash Attention for RoCm is bumped to 2.1. For details, please see the comment in QwenFlashAttention2 __init__.
...
class QwenSdpaAttention(QwenAttention):
    """
    Qwen attention module using torch.nn.functional.scaled_dot_product_attention. This module inherits from
    `QwenAttention` as the weights of the module stays untouched. The only changes are on the forward pass to adapt to
    SDPA API.
    """
    ...
    def forward(
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "QwenModel is using QwenSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True`. Falling back to the manual attention implementation, "

def check_weight(model, logger):
    logger.info("--- KIỂM TRA TRỌNG SỐ Encoder RMS NORM ---")
    ...
    logger.info(f"Encoder RMS norm Weight | Max: {rms_norm.weight.max().item():.4f} "
    ...
    if rms_norm.weight.std().item() > 0.1:
        logger.warning("❌ CẢNH BÁO: Trọng số quá lớn! Hàm khởi tạo chưa chạy đúng.")
    else:
        logger.info("✅ Trọng số có vẻ ổn (std nhỏ).")

    logger.info("--- KIỂM TRA TRỌNG SỐ PROJECT DOWN ---")
    # Thay đường dẫn tới module project_down của bạn cho đúng
    # Ví dụ: model.encoder.connector.encoder_project.linear_1
    linear1 = model.get_encoder().connector.encoder_project.linear_1 
    logger.info(f"Linear 1 Weight | Max: {linear1.weight.max().item():.4f} "
    ...
    logger.info(f"Linear 2 Weight | Max: {linear2.weight.max().item():.4f} "
    ...
    logger.info("--- KIỂM TRA TRỌNG SỐ CONNECTOR ---")
    ...
    logger.info("--- KIỂM TRA TRỌNG SỐ DECODER ---")
    ...
    logger.info(f"Decoder RMS norm Weight | Max: {decoder_norm.weight.max().item():.4f} "
    ...
    logger.info(f"Lm head Weight | Max: {lm_head.weight.max().item():.4f} "
